...
import asyncio
from OpenVLAModel import OpenVLAModel  # Импорт вашего класса LlavaModel
...
logger = CustomLogger(mode="logging", log_file="/llserver/logs/openvla.log")  # Можно переключить на "wandb"
...
async def lifespan(app: FastAPI):
    ...
    logger.log(f"Инициализация модели OpenVLAModel начинается...")

    model = OpenVLAModel(logger=logger)  # Передаем логгер в класс модели
    ...
    try:
        ...
        logger.log(f"Модель OpenVLAModel была успешно проинициализирована.")
        yield
    finally:
...
async def put_task(
    ...
    model: OpenVLAModel = Depends(get_model),
):
    """
